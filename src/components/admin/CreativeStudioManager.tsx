...
import { Loader2, Palette, Trash2, Edit2 } from 'lucide-react';

export default function CreativeStudioManager({ brandId }: { brandId: string | null }) {
  ...
  const fetchItems = async () => {
    let query = supabase.from('creative_studio').select('*, brands(name)').order('created_at', { ascending: false });
    if (brandId) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      const { error } = await supabase.from('creative_studio').update({
        title,
      ...
      const { error } = await supabase.from('creative_studio').insert(rows);
  ...
  const handleDelete = async (id: string) => {
    ...
    const { error } = await supabase.from('creative_studio').delete().eq('id', id);
    if (error) {
      alert('Error: ' + error.message);
    } else {
      fetchItems();
    ...
    <div className="space-y-8">
      {!brandId ? (
        <div className="bg-yellow-50 dark:bg-yellow-900/30 p-6 rounded-3xl border border-yellow-200 dark:border-yellow-700/50 text-yellow-800 dark:text-yellow-200">
          Please select or create a brand from the top header before managing creative assets.
        </div>
      ) : (
        <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
              <Palette className="text-indigo-500 dark:text-indigo-400" />
              {editingId ? 'Edit Creative Studio Item' : 'Add Creative Studio Item(s)'}
            </h3>
            {editingId && (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                Media Upload {editingId ? '' : '(Bulk supported)'}
              </label>
              {mediaUrls.length > 0 ? (
                ...
                <MediaUploader onUploadSuccess={setMediaUrls} acceptedTypes={mediaType === 'image' ? 'image/*' : 'video/*'} multiple={!editingId} />
              )}
            </div>
            ...
            >
              ...
              {editingId ? 'Update Item' : `Save ${mediaUrls.length > 1 ? `${mediaUrls.length} Items` : 'Item'}`}
            </button>
          </form>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
          <h3 className="font-bold text-slate-900 dark:text-white">Recent Items {brandId && '(This Brand)'}</h3>
        </div>
          ...
          {items.map(item => (
            <div key={item.id} className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-800 overflow-hidden shadow-sm relative group">
              ...
              {item.media_type === 'image' ? (
                ...
                <video src={item.media_url} className="w-full h-48 object-cover" muted />
              )}
              <div className="p-4">
                <h4 className="font-bold text-slate-900 dark:text-white">{item.title}</h4>
                <p className="text-xs text-slate-500 dark:text-slate-400 uppercase mt-1">{item.media_type}</p>
              </div>
            </div>
          ))}
          {items.length === 0 && <p className="text-slate-500 dark:text-slate-400 text-sm col-span-full">No items uploaded yet.</p>}
        </div>
      </div>
    </div>
  );
